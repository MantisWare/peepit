...
const INVALID_PATH_SENTINEL = "PEEPIT_CLI_PATH_RESOLUTION_FAILED";

function determineSwiftCliPath(packageRootDirForFallback?: string): string {
  const envPath = process.env.PEEPIT_CLI_PATH;
  if (envPath) {
    try {
  ...
  if (packageRootDirForFallback) {
    return path.resolve(packageRootDirForFallback, "peepit");
  }

  // If neither PEEPIT_CLI_PATH is valid nor packageRootDirForFallback is provided,
...
export function initializeSwiftCliPath(packageRootDir: string): void {
  if (!packageRootDir) {
    // If PEEPIT_CLI_PATH is also not set or invalid, this will lead to INVALID_PATH_SENTINEL
...
function getInitializedSwiftCliPath(logger: Logger): string {
  ...
  if (!resolvedCliPath || resolvedCliPath === INVALID_PATH_SENTINEL) {
    const errorMessage = "PeepIt Swift CLI path is not properly initialized or resolution failed. " +
      `Resolved path: '${resolvedCliPath}'. Ensure PEEPIT_CLI_PATH is valid or ` +
...
export async function execPeepIt(
  args: string[],
  packageRootDir: string,
  options: { expectSuccess?: boolean; timeout?: number } = {},
): Promise<{ success: boolean; data?: string; error?: string }> {
  const cliPath = process.env.PEEPIT_CLI_PATH || path.resolve(packageRootDir, "peepit");
