...
async function checkOllamaStatus(model: string, logger: Logger): Promise<ProviderStatus> {
  try {
    const baseUrl = process.env.PEEPIT_OLLAMA_BASE_URL || "http://localhost:11434";
...
export async function determineProviderAndModel(
  ...
  if (requestedProviderType !== "auto") {
    ...
    if (!configuredProvider) {
      throw new Error(
        `Provider '${requestedProviderType}' is not enabled in server's PEEPIT_AI_PROVIDERS configuration.`,
      );
    }
