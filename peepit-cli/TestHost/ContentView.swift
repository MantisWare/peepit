...
struct ContentView: View {
    ...
    @State private var peepitCliAvailable = false

    private let testIdentifier = "PeepItTestHost"

    var body: some View {
        VStack(spacing: 20) {
            // Header
            Text("PeepIt Test Host")
                .font(.largeTitle)
                .padding(.top)
            ...
            GroupBox("Permissions") {
                VStack(alignment: .leading, spacing: 10) {
                    ...
                    HStack {
                        Image(systemName: peepitCliAvailable ? "checkmark.circle.fill" : "xmark.circle.fill")
                            .foregroundColor(peepitCliAvailable ? .green : .red)
                        Text("PeepIt CLI")
                        Spacer()
                        Button("Check") {
                            checkPeepItCli()
    ...
    private func checkPeepItCli() {
        let cliPath = "../.build/debug/peepit"
        if FileManager.default.fileExists(atPath: cliPath) {
            peepitCliAvailable = true
            addLog("PeepIt CLI found at: \(cliPath)")
        } else {
            peepitCliAvailable = false
            addLog("PeepIt CLI not found. Run 'swift build' first.")
        }
    }
