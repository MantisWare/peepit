...
struct ScreenshotValidationTests {
    ...
    func validateScreenshotContent() async throws {
        ...
        let testWindow = createTestWindow(withContent: .text("PEEPIT_TEST_12345"))
        ...
        let outputPath = "/tmp/peepit-content-test.png"
    ...
    func visualRegressionTest() async throws {
        ...
        // Capture baseline
        let baselinePath = "/tmp/peepit-baseline.png"
        let currentPath = "/tmp/peepit-current.png"
        defer {
    ...
    func imageFormats() async throws {
        ...
        for format in formats {
            let path = "/tmp/peepit-format-test.\(format.rawValue)"
    ...
    func multiDisplayCapture() async throws {
        ...
        for (index, screen) in screens.enumerated() {
            ...
            let outputPath = "/tmp/peepit-display-\(index).png"
            defer { try? FileManager.default.removeItem(atPath: outputPath) }
    ...
    func capturePerformance() async throws {
        ...
        for iteration in 0..<iterations {
            let path = "/tmp/peepit-perf-\(iteration).png"
            defer { try? FileManager.default.removeItem(atPath: path) }
    ...
    private func createTestWindow(withContent content: TestContent) -> NSWindow {
        ...
        window.title = "PeepIt Test Window"
