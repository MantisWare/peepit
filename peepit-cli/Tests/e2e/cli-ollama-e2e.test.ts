...
// Helper function to run PeepIt CLI commands
const runPeepItCli = (args: string[]): Promise<{ stdout: string; stderr: string; code: number | null }> => {
  return new Promise((resolve) => {
    const peepitPath = path.resolve(__dirname, '../../../../peepit'); // Assuming CLI is in project root
    const process = child_process.spawn(peepitPath, args);
    let stdout = '';
    let stderr = '';
    ...
    process.on('error', (err) => {
      resolve({ stdout: '', stderr: `Failed to start PeepIt CLI: ${err.message}`, code: -1 });
    });
  });
};
...
const describeAppImageCaptureTest = (appName: string) => {
  describe(`${appName} CLI Image Capture E2E Test`, () => {
    let tempImagePath = '';
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'peepit-e2e-'));
    ...
    test(`should take a screenshot of ${appName} using the CLI`, async () => {
      ...
      const imageResult = await runPeepItCli(['image', '--app', appName, '--path', tempImagePath]);
      ...
      // Specific error messages from peepit CLI can be checked here if known
      ...
      // Optional: Check stdout for confirmation message if peepit CLI provides one
    ...
    //   const analyzeResult = await runPeepItCli(['analyze', '--image-path', tempImagePath, '--question', question]);
...
// 1. PeepIt CLI (the Swift executable) is built and accessible at the root of the project.
