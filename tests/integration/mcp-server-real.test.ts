...
describe.skip("MCP Server Real Integration Tests", () => {
  ...
  beforeAll(async () => {
    ...
    const peepitCliUtils = await import("../../src/utils/peepit-cli");
    mockedExecuteSwiftCli =
      peepitCliUtils.executeSwiftCli as unknown as vi.MockInstance;
    // peepitCliUtils.initializeSwiftCliPath(); // Call the mocked version if needed by the test setup
  });
  ...
  describe("Error Recovery and Edge Cases", () => {
    it("should handle Swift CLI timeout gracefully", async () => {
      const { executeSwiftCli } = require("../../src/utils/peepit-cli");
