...
import { readImageAsBase64 } from "../../src/utils/peepit-cli";
...
describeSwiftTests("analyzeToolHandler Integration Tests", () => {
  ...
  beforeEach(async () => {
    ...
    process.env.PEEPIT_OLLAMA_BASE_URL = "http://localhost:11434";
  });

  it("should return error if PEEPIT_AI_PROVIDERS is not configured (env var is empty)", async () => {
  ...
  it("should return error if image_path has an unsupported extension", async () => {
    process.env.PEEPIT_AI_PROVIDERS = "mocked_providers_exist"; // Allow to proceed past initial check
    currentParseAIProvidersImpl = () => [
  ...
  it("should successfully call specified provider (Ollama) if available", async () => {
    process.env.PEEPIT_AI_PROVIDERS = "mocked_providers_exist"; // Allow to proceed
  ...
  it("should successfully call specified provider (OpenAI) if available, using model from PEEPIT_AI_PROVIDERS if not specified in call", async () => {
    process.env.PEEPIT_AI_PROVIDERS = "mocked_providers_exist"; // Allow to proceed
  ...
  it("should use auto provider selection (first available from PEEPIT_AI_PROVIDERS)", async () => {
    process.env.PEEPIT_AI_PROVIDERS = "mocked_providers_exist"; // Allow to proceed
  ...
  it("should use default model for a provider if not specified in PEEPIT_AI_PROVIDERS string and not in call", async () => {
    process.env.PEEPIT_AI_PROVIDERS = "mocked_providers_exist"; // Allow to proceed
  ...
  it("should return error if parseAIProviders returns empty even if PEEPIT_AI_PROVIDERS env is set", async () => {
    process.env.PEEPIT_AI_PROVIDERS = "some,csv,string"; // Env var is set
    ...
    const content = response.content as Array<{ type: string; text?: string }>;
    expect(content[0]?.text).toBe(
      "No valid AI providers found in PEEPIT_AI_PROVIDERS configuration.",
    );
  });
});
