...
import { initializeSwiftCliPath } from "../../src/utils/peepit-cli";
import { Result } from "@modelcontextprotocol/sdk/types.js"; // Corrected SDK import path and type

// Define a more specific type for content items used in PeepIt
interface PeepItContentItem {
  type: string;
  text?: string;
  imageUrl?: string;
  data?: any;
...
// Initialize Swift CLI path (assuming 'peepit' binary is at project root)
  ...
  describe("listToolHandler", () => {
    it("should return server_status correctly", async () => {
      ...
      if (
        ...
        const firstContentItem = response.content[0] as PeepItContentItem;
        expect(firstContentItem.type).toBe("text");
        expect(firstContentItem.text).toContain("PeepIt MCP");
      } else {
        expect(
          false,
    ...
    it("should list windows for a known application (Finder) without details by default", async () => {
      ...
      if (!response.isError) {
        ...
        const windowList = (response as any)
          .window_list as PeepItWindowItem[];
  ...
  describe("imageToolHandler", () => {
    ...
    beforeEach(() => {
      tempImagePath = path.join(
        os.tmpdir(),
        `peepit-test-image-${Date.now()}.png`,
      );
    });
