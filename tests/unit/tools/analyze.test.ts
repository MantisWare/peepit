...
describe("Analyze Tool", () => {
  ...
  describe("analyzeToolHandler", () => {
    ...
    it("should analyze image successfully with auto provider selection", async () => {
      ...
      expect(result.content[1].text).toMatch(/👻 PeepIt: Analyzed image with openai\/gpt-4o in \d+\.\d+s\./);
    ...
    it("should use specific provider and model if provided and available", async () => {
      process.env.PEEPIT_AI_PROVIDERS = "openai/gpt-4-turbo";
      const parsedProviders: AIProvider[] = [
      ...
      expect(result.content[1].text).toMatch(/👻 PeepIt: Analyzed image with openai\/gpt-custom-model in \d+\.\d+s\./);      
    ...
    it("should return error if PEEPIT_AI_PROVIDERS env is not set", async () => {
    ...
    it("should return error if PEEPIT_AI_PROVIDERS env has no valid providers", async () => {
      process.env.PEEPIT_AI_PROVIDERS = "invalid/";
    ...
    it("should return error if specific provider in config is not enabled on server", async () => {
      process.env.PEEPIT_AI_PROVIDERS = "ollama/llava"; // Server only has ollama
      mockParseAIProviders.mockReturnValue([
        { provider: "ollama", model: "llava" },
      ]);
      mockDetermineProviderAndModel.mockRejectedValue(
        new Error(
          "Provider 'openai' is not enabled in server's PEEPIT_AI_PROVIDERS configuration."
        )
      );
    ...
    it("should handle provider returning empty string", async () => {
      ...
      expect(result.content[1].text).toMatch(/👻 PeepIt: Analyzed image with ollama\/llava in \d+\.\d+s\./);            
