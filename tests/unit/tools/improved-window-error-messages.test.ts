...
import { executeSwiftCli } from "../../../src/utils/peepit-cli";
...
describe("Improved Window Error Messages", () => {
  ...
  it("should handle case where no similar windows exist", async () => {
    const mockNoSimilarWindowsResponse = {
      success: false,
      error: {
        message: "Window with title containing 'very-specific-search' not found in Code. Available windows: \"ImageCommand.swift - peepit\", \"main.swift - peepit\". Note: For URLs, try without the protocol (e.g., 'example.com:8080' instead of 'http://example.com:8080').",
        code: "WINDOW_NOT_FOUND",
    ...
    expect(errorText).toContain("ImageCommand.swift - peepit");
    expect(errorText).toContain("main.swift - peepit");
  });
