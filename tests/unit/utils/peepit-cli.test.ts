...
describe("Swift CLI Utility", () => {
  ...
  const DEFAULT_CLI_PATH_IN_PACKAGE = path.join(MOCK_PACKAGE_ROOT, "peepit");
  const CUSTOM_CLI_PATH = "/custom/path/to/peepit";

  let mockedFsExistsSync: vi.Mock; // To store the mock instance

  beforeEach(async () => {
    vi.clearAllMocks();
    process.env.PEEPIT_CLI_PATH = "";
  ...
  describe("executeSwiftCli with path resolution", () => {
    it("should use CLI path from PEEPIT_CLI_PATH if set and valid", async () => {
      process.env.PEEPIT_CLI_PATH = CUSTOM_CLI_PATH;
    ...
    it("should use bundled path if PEEPIT_CLI_PATH is set but invalid", async () => {
      process.env.PEEPIT_CLI_PATH = "/invalid/path/peepit";
    ...
    it("should use bundled path derived from packageRootDir if PEEPIT_CLI_PATH is not set", async () => {
      initializeSwiftCliPath(MOCK_PACKAGE_ROOT);
      // No need to mock existsSync here if PEEPIT_CLI_PATH is empty, as it won't be checked for that path.
  ...
  describe("executeSwiftCli command execution and output parsing", () => {
    beforeEach(async () => {
      const fs = await import("fs"); // Import the mocked fs module here
      mockedFsExistsSync = fs.existsSync as vi.Mock;
      mockedFsExistsSync.mockReturnValue(false); // Ensure PEEPIT_CLI_PATH (if set) is seen as invalid
  ...
  describe("executeSwiftCli exit code mapping", () => {
    ...
    it("should handle an unknown exit code with a generic message", async () => {
      ...
      expect(result.error?.message).toBe("PeepIt CLI Error: Some stderr text");
    });
  });
});
