...
describe("AI Providers Utility", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    delete process.env.PEEPIT_OLLAMA_BASE_URL;
  ...
  describe("isProviderAvailable", () => {
    ...
    it("should use PEEPIT_OLLAMA_BASE_URL for Ollama check", async () => {
      process.env.PEEPIT_OLLAMA_BASE_URL = "http://custom-ollama:11434";
      (global.fetch as vi.Mock).mockResolvedValue({
        ok: true,
        json: () => Promise.resolve({
          models: [
  ...
  describe("determineProviderAndModel", () => {
    ...
    it("should throw if requested provider is not configured", async () => {
      await expect(
        ...
        "Provider 'anthropic' is not enabled in server's PEEPIT_AI_PROVIDERS configuration.",
      );
    });
