#!/bin/bash
################################################################################
# test_peepit.sh - Comprehensive test suite for PeepIt screenshot automation
...
PEEPIT_SCRIPT="$SCRIPT_DIR/peepit.scpt"
# Legacy variables for backward compatibility
PEEPIT_CLASSIC="$PEEPIT_SCRIPT"
PEEPIT_PRO="$PEEPIT_SCRIPT"
TEST_OUTPUT_DIR="$HOME/Desktop/peepit_tests"
...
run_test() {
    ...
    if [[ "$expected_result" == "success" ]]; then
        ...
        if [[ $exit_code -ne 0 ]] || [[ "$result" == *"Error:"* ]] || [[ "$result" == *"PeepIt 👀:"* ]] && [[ "$result" == *"Error"* ]]; then
...
create_test_image() {
    ...
    if command -v magick >/dev/null 2>&1; then
        magick -size 400x300 xc:white -fill black -pointsize 30 -annotate +50+150 "PeepIt Test Image" "$image_path"
    elif command -v convert >/dev/null 2>&1; then
        convert -size 400x300 xc:white -fill black -pointsize 30 -annotate +50+150 "PeepIt Test Image" "$image_path"
    else
        # Fallback: take a screenshot of Finder to create test image
        osascript "$PEEPIT_SCRIPT" "Finder" "$image_path" >/dev/null 2>&1 || true
    fi
}
...
run_ai_test() {
    ...
    case "$test_type" in
        ...
        "two-step")
            ...
            local test_image="/tmp/peepit_ai_test_${TIMESTAMP}.png"
...
setup_test_environment() {
    log_info "Setting up PeepIt test environment..."
    
    # Check if PeepIt scripts exist
    if [[ ! -f "$PEEPIT_CLASSIC" ]]; then
        log_error "PeepIt Classic script not found at: $PEEPIT_CLASSIC"
        exit 1
    fi
    
    if [[ ! -f "$PEEPIT_PRO" ]]; then
        log_error "PeepIt Pro script not found at: $PEEPIT_PRO"
        exit 1
    fi
...
run_basic_tests() {
    ...
    if result=$(osascript "$PEEPIT_SCRIPT" Finder -q 2>&1); then
        if [[ "$result" =~ ^/tmp/peepit_finder_[0-9_]+\.png$ ]]; then
    ...
    if result=$(osascript "$PEEPIT_SCRIPT" -q 2>&1); then
        if [[ "$result" =~ ^/tmp/peepit_fullscreen_[0-9_]+\.png$ ]]; then
    ...
    if result=$(osascript "$PEEPIT_SCRIPT" Safari -o "$custom_path" -q 2>&1); then
    ...
    if result=$(osascript "$PEEPIT_SCRIPT" com.apple.finder -q 2>&1); then
        if [[ "$result" =~ ^/tmp/peepit_com_apple_finder_[0-9_]+\.png$ ]]; then
...
run_window_capture_tests() {
    ...
    if result=$(osascript "$PEEPIT_SCRIPT" Finder -o "$window_test_path" -v 2>&1); then
    ...
    # Capture window
    osascript "$PEEPIT_SCRIPT" Safari -o "$window_file" -q 2>&1
    # Capture fullscreen
    osascript "$PEEPIT_SCRIPT" -f -o "$fullscreen_file" -q 2>&1
    ...
    if [[ -n "$multi_app" ]]; then
        if result=$(osascript "$PEEPIT_SCRIPT" "$multi_app" -m -v 2>&1); then
    ...
    if result=$(osascript "$PEEPIT_SCRIPT" "NonExistentApp123" 2>&1); then
        if [[ "$result" == *"PeepIt 👀:"* ]]; then
            log_success "Fallback message - Uses correct PeepIt prefix"
        else
            log_error "Fallback message - Missing PeepIt prefix"
        fi
    else
    ...
    if command -v ollama >/dev/null 2>&1 && [[ $(get_test_vision_models | wc -l) -gt 0 ]]; then
        ...
        if result=$(osascript "$PEEPIT_SCRIPT" Finder -a "Test question" -v 2>&1); then
...
run_format_tests() {
    ...
    if result=$(osascript "$PEEPIT_SCRIPT" Finder -o "$png_path" -q 2>&1); then
        if [[ -f "$png_path" ]]; then
    ...
    if result=$(osascript "$PEEPIT_SCRIPT" Finder -o "$jpg_base" --format jpg -q 2>&1); then
    ...
    if result=$(osascript "$PEEPIT_SCRIPT" Finder -o "$pdf_path" -q 2>&1); then
        if [[ -f "$pdf_path" ]]; then
...
run_advanced_tests() {
    ...
    if result=$(osascript "$PEEPIT_SCRIPT" Finder -m -o "$TEST_OUTPUT_DIR/" 2>&1); then
        # Check if multiple files were created
        local window_files=$(ls "$TEST_OUTPUT_DIR"/peepit_finder_*_window_*.png 2>/dev/null | wc -l)
    ...
    if result=$(osascript "$PEEPIT_SCRIPT" Safari -f -q 2>&1); then
    ...
    if result=$(osascript "$PEEPIT_SCRIPT" Finder -v -q 2>&1); then
    ...
    if result=$(osascript "$PEEPIT_SCRIPT" TextEdit -w -o "$combo_path" --format jpg -v -q 2>&1); then
...
run_discovery_tests() {
    ...
    if result=$(osascript "$PEEPIT_SCRIPT" list 2>&1); then
    ...
    if result=$(osascript "$PEEPIT_SCRIPT" ls 2>&1); then
        if [[ "$result" == *"Running Applications:"* ]]; then
    ...
    if result=$(osascript "$PEEPIT_SCRIPT" help 2>&1); then
    ...
    if result=$(osascript "$PEEPIT_SCRIPT" -h 2>&1); then
...
run_ollama_tests() {
    ...
    if osascript "$PEEPIT_SCRIPT" "Finder" "$existing_screenshot" >/dev/null 2>&1; then
    ...
    if [[ -n "$multi_window_app" ]]; then
        ...
        if result=$(osascript "$PEEPIT_SCRIPT" "$multi_window_app" -w -a "What's on this tab?" 2>&1); then
    ...
    local result
    if result=$(osascript "$PEEPIT_SCRIPT" "Finder" "--ask" 2>&1); then
...
run_claude_tests() {
    ...
    local result
    if result=$(osascript "$PEEPIT_SCRIPT" "Safari" "--ask" "Test" "--provider" "claude" 2>&1); then
...
run_provider_selection_tests() {
    ...
    local result
    if result=$(osascript "$PEEPIT_SCRIPT" "Finder" "--ask" "What is shown?" 2>&1); then
        if [[ "$ollama_available" == true ]]; then
...
run_enhanced_messaging_tests() {
    ...
    if app_error_result=$(osascript "$PEEPIT_CLASSIC" "FakeApp123" "/tmp/test.png" 2>&1); then
    ...
    if bundle_error_result=$(osascript "$PEEPIT_PRO" "com.fake.bundle" "/tmp/test.png" 2>&1); then
    ...
    if success_result=$(osascript "$PEEPIT_CLASSIC" "Finder" "/tmp/peepit_message_test.png" 2>&1); then
        if [[ "$success_result" == *"Screenshot captured successfully!"* ]] && [[ "$success_result" == *"• File:"* ]] && [[ "$success_result" == *"💡"* ]]; then
            ...
            rm -f "/tmp/peepit_message_test.png"
        else
...
run_all_tests() {
    log_info "🎪 Starting Comprehensive PeepIt Test Suite 🎪"
    echo "Timestamp: $TIMESTAMP"
    echo "Test output directory: $TEST_OUTPUT_DIR"
    echo "Classic script: $PEEPIT_CLASSIC"
    echo "Pro script: $PEEPIT_PRO"
    echo ""
...
show_usage_tests() {
    ...
    local help_output
    if help_output=$(osascript "$PEEPIT_SCRIPT" help 2>&1); then
        if [[ "$help_output" == *"USAGE:"* ]] && [[ "$help_output" == *"PeepIt"* ]]; then
    ...
    if no_args_output=$(osascript "$PEEPIT_SCRIPT" -q 2>&1); then
        if [[ "$no_args_output" =~ ^/tmp/peepit_fullscreen_[0-9_]+\.png$ ]]; then
...
run_integration_tests() {
    ...
    local app_list
    if app_list=$(osascript "$PEEPIT_PRO" list 2>&1); then
...
show_summary() {
    ...
    if [[ $TESTS_FAILED -eq 0 ]]; then
        log_success "🎉 ALL TESTS PASSED! PeepIt is ready to rock! 🎉"
        echo "👀 → 📸 → 💾 — PeepIt is working perfectly!"
    else
...
case "${1:-}" in
    --help|-h)
        echo "🎪 PeepIt Comprehensive Test Suite 🎪"
        echo ""
        ...
        echo "- peepit.scpt in same directory"
