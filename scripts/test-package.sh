...
# Simple package test script for PeepIt MCP
...
if [ -f "node_modules/@mantisware/peepit-mcp/peepit" ]; then
    echo "✅ Binary found"
    
    # Check if executable
    if [ -x "node_modules/@mantisware/peepit-mcp/peepit" ]; then
        echo "✅ Binary is executable"
        
        # Test the binary
        echo ""
        echo "🧪 Testing Swift CLI..."
        if node_modules/@mantisware/peepit-mcp/peepit --version; then
...
if [ -f "node_modules/@mantisware/peepit-mcp/dist/index.js" ]; then
...
find node_modules/@mantisware/peepit-mcp -type f -name "*.js" -o -name "*.d.ts" -o -name "peepit" | head -20
...
const mcpPath = path.join('node_modules', '@mantisware', 'peepit-mcp', 'dist', 'index.js');
