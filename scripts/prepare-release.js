...
/**
 * Release preparation script for @mantisware/peepit-mcp
 * 
...
function checkSwift() {
  ...
  try {
    ...
    swiftBuildOutput = execSync('cd peepit-cli && swift build --arch arm64 -c release 2>&1', {
      cwd: projectRoot,
      encoding: 'utf8'
    });
  } catch (error) {
  ...
  // Test help command
  const helpOutput = exec('./peepit --help', { allowFailure: true });
  ...
  const versionOutput = exec('./peepit --version', { allowFailure: true });
  if (!versionOutput) {
  ...
  const appsOutput = exec('./peepit list apps --json-output', { allowFailure: true });
  if (!appsOutput) {
  ...
  const windowsOutput = exec('./peepit list windows --app Finder --json-output', { allowFailure: true });
  if (!windowsOutput) {
  ...
  const errorOutput = exec('./peepit list windows --app NonExistentApp12345 --json-output 2>&1', { allowFailure: true });
  if (errorOutput) {
    try {
  ...
  const imageHelpOutput = exec('./peepit image --help', { allowFailure: true });
...
function checkSwiftCLIIntegration() {
  ...
  let invalidOutput;
  try {
    execSync('./peepit invalid-command 2>&1', { 
      cwd: projectRoot,
      encoding: 'utf8',
      stdio: 'pipe'
    });
  ...
  try {
    missingArgsOutput = execSync('./peepit image --mode window --json-output 2>&1', { 
      cwd: projectRoot,
      encoding: 'utf8',
      stdio: 'pipe'
    });
  ...
  try {
    execSync('./peepit image --mode window --app Finder --window-index abc --json-output 2>&1', { 
      cwd: projectRoot,
      encoding: 'utf8',
      stdio: 'pipe'
    });
  ...
  for (const cmd of subcommands) {
    const helpOutput = exec(`./peepit ${cmd} --help`, { allowFailure: true });
    if (!helpOutput || !helpOutput.includes('USAGE')) {
  ...
  const formats = [
    { cmd: './peepit list apps --json-output', required: ['success', 'data'] }
  ];
  ...
  const captureTest = exec('./peepit image --mode screen --json-output', { allowFailure: true });
  if (captureTest) {
    try {
...
function buildAndVerifyPackage() {
  ...
  // Verify peepit binary
  log('Verifying peepit binary...', colors.cyan);
  const binaryPath = join(projectRoot, 'peepit');
  
  // Check if binary exists
  if (!existsSync(binaryPath)) {
    logError('peepit binary not found');
    return false;
  }
  ...
  try {
    ...
    if ((perms & 0o111) === 0) {
      logError('peepit binary is not executable');
      return false;
    }
  } catch (error) {
  ...
  try {
    ...
    if (!lipoOutput.includes('arm64') || !lipoOutput.includes('x86_64')) {
      logError('peepit binary does not contain both architectures (arm64 and x86_64)');
  ...
  try {
    ...
    if (!helpOutput || helpOutput.length === 0) {
      logError('peepit binary does not respond to --help command');
      return false;
    }
    logSuccess('Binary responds correctly to --help command');
  } catch (error) {
    logError('peepit binary failed to execute with --help');
    logError(`Error: ${error.message}`);
    return false;
  }
  
  logSuccess('peepit binary verification passed');
...
// Main execution
async function main() {
  console.log(`\n${colors.bright}🚀 PeepIt MCP Release Preparation${colors.reset}\n`);
