#!/usr/bin/osascript
--------------------------------------------------------------------------------
-- peepit.scpt - v1.0.0 "PeepIt Pro! 👀 → 📸 → 💾"
-- Enhanced screenshot capture with multi-window support and app discovery
-- PeepIt—screenshot got you! Now you see it, now it's saved.
--
...
property scriptInfoPrefix : "PeepIt 👀: "
...
on getOllamaInstallInstructions()
    ...
    set instructions to instructions & "Then retry your PeepIt command with --ask or --analyze!"
    return instructions
...
on analyzeImageWithOllama(imagePath, question, requestedModel)
    ...
    try
        ...
        if imageSizeBytes > 5000000 then -- 5MB threshold
            ...
            set compressedPath to "/tmp/peepit_ai_compressed.png"
        ...
        set jsonTempFile to "/tmp/peepit_ollama_request.json"
        try
...
on getAppWindowStatus(appName)
    ...
    if totalWindows = 0 then
        return my formatErrorMessage("No Windows Error", "App '" & appName & "' is running but has no windows open. PeepIt needs at least one window to capture. Please open a window in " & appName & " and try again.", "zero windows")
    end if
